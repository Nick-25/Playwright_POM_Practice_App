...
API endpoints, browser tests, API tests, CI execution, HTML artifacts, and
standard CTRF test reporting.
...
- Standard CTRF reporting for passed, failed, flaky, skipped, retry, and
  duration information in the GitHub Actions run summary
...
- CTRF output generated by `playwright-ctrf-json-reporter`
- GitHub Actions test reporting through `ctrf-io/github-test-reporter`
...
2. `Run Playwright Tests` installs Playwright browsers and runs `npm test`
3. `Publish Test Report` downloads the CTRF artifact and publishes the test
   report to the GitHub Actions summary

The test job creates:

- `playwright-report/` for the Playwright HTML report
- `test-results/` for Playwright traces and attachments
- `ctrf/ctrf-report.json` for standard CTRF reporting

The report job uses `ctrf-io/github-test-reporter@v1` to show:

- Overall pass/fail status
- Passed, failed, flaky, skipped, retry, and duration data
...
This creates the Playwright HTML report in `playwright-report/` and the CTRF
report in `ctrf/ctrf-report.json`.
